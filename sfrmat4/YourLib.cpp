    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("YourLib"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return YourLibInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
    return mclFeval(_mcr_inst, "sfrmat5", nlhs, plhs, nrhs, prhs);
}

LIB_YourLib_CPP_API 
void MW_CALL_CONV sfrmat5(int nargout, mwArray& mtf502, const mwArray& a)
{
    mclcppMlfFeval(_mcr_inst, "sfrmat5", nargout, 1, 1, &mtf502, &a);
}
