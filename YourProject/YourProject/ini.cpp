...
void readConfig(HOUGH * h,MTF * m) {
	...
	const char *minR = ini_get(config, "Hough", "minRadius");
	h->minRadius = atof(minR);

	const char *maxR = ini_get(config, "Hough", "maxRadius");
	h->maxRadius = atof(maxR);

	const char *cparam2 = ini_get(config, "Hough", "param2");
	h->param2 = atof(cparam2);

	const char *cparam1 = ini_get(config, "Hough", "param1");
	h->param1 = atof(cparam1);

	const char *cminDist = ini_get(config, "Hough", "minDist");
	h->minDist = atof(cminDist);


	const char *cxdistance = ini_get(config, "mtf", "xdistance");
	m->xdistance = atof(cxdistance);

	const char *cydistance = ini_get(config, "mtf", "ydistance");
	m->ydistance = atof(cydistance);

	const char *cwidth = ini_get(config, "mtf", "width");
	m->width = atof(cwidth);

	const char *chigh = ini_get(config, "mtf", "high");
	m->high = atof(chigh);
